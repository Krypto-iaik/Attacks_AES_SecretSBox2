...
void computationMDS3(word8 a[MAX][MAX])
{
    int i, j, k, h, jj, hh;
    word8 det = 0x00, temp[MAX][MAX];
                ...
                for(dd = 0; dd<256; dd++)
                {
                    ...
                    //if((aa != 0) && (bb != 0) && (cc != 0) && (dd != 0))
                    {
                        ...
                        if(det != 0x00)
                        {
                            ...
                            {
                                computationMDS3(matrix);
                                ec = equalCoeff(invA, invB, invC, invD);
                                if(ec == 1)
                                {
                                    eqCoef = eqCoef + 1;
                                    zeroSum = zeroSum + 1;
                                ...
                                {
                                    if(sumCoeffZero(a, b, c, d) == 1)
                                        zeroSum = zeroSum + 1;
                                    else
                                        zeroSum = zeroSum + sumCoeffZero(invA, invB, invC, invD);
                                }
                            }
                        }
                    }
...
/**This program counts the number of 4*4 Circulant Matrices MC with word of 8 bits with the following properties:

1)  - Invertible
    - Invertible and two equal coefficients of MC or MC-1 in each row
    - Invertible and XoR-sum of >=2 coefficients of MC or MC-1 equal to zero in each row

2)  - MDS
    - MDS and two equal coefficients of MC or MC-1 in each row
    - MDS and XoR-sum of >=2 coefficients of MC or MC-1 equal to zero in each row
*/
...
int main()
{
    ...
    fp = fopen("Matrix_8bit_Inv.txt","w+");
    ...
    if(MDS == 0)
        fprintf(fp, "Invertible: %f\n - Invertible and two equal coefficients of MC or MC-1 in each row: %f\n - Invertible and XoR-sum of >=2 coefficients of MC or MC-1 equal to zero in each row: %f\n\n", invMatrix, eqCoef, zeroSum);
    else
        fprintf(fp, "MDS: %f\n - MDS and two equal coefficients of MC or MC-1 in each row: %f\n - MDS and XoR-sum of >=2 coefficients of MC or MC-1 equal to zero in each row: %f\n\n", invMatrix, eqCoef, zeroSum);
